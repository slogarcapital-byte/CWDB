...
try {
    ...
    $row = @(Get-SupabaseRows -Table "approval_queue" -Select "approval_id,status" -Filter "summary=eq.patch-returning test $uid")[0]
...
try {
    ...
    $task = @(Get-SupabaseRows -Table "task" -Select "task_id,status,payload" -Filter "title=eq.decide-test task $tuid")[0]

    Invoke-SupabaseInsert -Table "approval_queue" -Records @(@{
        action_kind = 'dashboard-selftest'; summary = "decide test $tuid"; task_id = $task.task_id
        proposed_action = @{ test = $true }; status = 'pending'
    })
    $appr = @(Get-SupabaseRows -Table "approval_queue" -Select "approval_id" -Filter "summary=eq.decide test $tuid")[0]
    ...
    $apprAfter = @(Get-SupabaseRows -Table "approval_queue" -Select "status,decision_note,decided_by" -Filter "approval_id=eq.$($appr.approval_id)")[0]
    ...
    $taskAfter = @(Get-SupabaseRows -Table "task" -Select "status,payload" -Filter "task_id=eq.$($task.task_id)")[0]
    ...
    $taskApp = @(Get-SupabaseRows -Table "task" -Select "task_id,status" -Filter "title=eq.approve-test task $tuidApp")[0]
    Invoke-SupabaseInsert -Table "approval_queue" -Records @(@{
        action_kind = 'dashboard-selftest'; summary = "approve test $tuidApp"; task_id = $taskApp.task_id
        proposed_action = @{ test = $true }; status = 'pending'
    })
    $apprApp = @(Get-SupabaseRows -Table "approval_queue" -Select "approval_id" -Filter "summary=eq.approve test $tuidApp")[0]
    ...
    $apprAppAfter = @(Get-SupabaseRows -Table "approval_queue" -Select "status" -Filter "approval_id=eq.$($apprApp.approval_id)")[0]
    Assert ($apprAppAfter.status -eq 'approved') "approve path: approval status=approved"
    $taskAppAfter = @(Get-SupabaseRows -Table "task" -Select "status" -Filter "task_id=eq.$($taskApp.task_id)")[0]
    ...
    $taskRej = @(Get-SupabaseRows -Table "task" -Select "task_id,status" -Filter "title=eq.reject-test task $tuidRej")[0]
    Invoke-SupabaseInsert -Table "approval_queue" -Records @(@{
        action_kind = 'dashboard-selftest'; summary = "reject test $tuidRej"; task_id = $taskRej.task_id
        proposed_action = @{ test = $true }; status = 'pending'
    })
    $apprRej = @(Get-SupabaseRows -Table "approval_queue" -Select "approval_id" -Filter "summary=eq.reject test $tuidRej")[0]
    ...
    $apprRejAfter = @(Get-SupabaseRows -Table "approval_queue" -Select "status" -Filter "approval_id=eq.$($apprRej.approval_id)")[0]
    Assert ($apprRejAfter.status -eq 'rejected') "reject path: approval status=rejected"
    $taskRejAfter = @(Get-SupabaseRows -Table "task" -Select "status" -Filter "task_id=eq.$($taskRej.task_id)")[0]
    ...
    $taskNoNote = @(Get-SupabaseRows -Table "task" -Select "task_id,status" -Filter "title=eq.nonote-test task $tuidNoNote")[0]
    Invoke-SupabaseInsert -Table "approval_queue" -Records @(@{
        action_kind = 'dashboard-selftest'; summary = "nonote test $tuidNoNote"; task_id = $taskNoNote.task_id
        proposed_action = @{ test = $true }; status = 'pending'
    })
    $apprNoNote = @(Get-SupabaseRows -Table "approval_queue" -Select "approval_id" -Filter "summary=eq.nonote test $tuidNoNote")[0]
    ...
    $apprNoNoteAfter = @(Get-SupabaseRows -Table "approval_queue" -Select "status" -Filter "approval_id=eq.$($apprNoNote.approval_id)")[0]
...
try {
    ...
    $drow = @(Get-SupabaseRows -Table "directive" -Select "*" -Filter "directive_id=eq.$($d.directive_id)")[0]
    ...
    $trow = @(Get-SupabaseRows -Table "task" -Select "status,priority,assigned_agent,payload,trace_id" -Filter "task_id=eq.$($inj.task_id)")[0]
    ...
    $drow2 = @(Get-SupabaseRows -Table "directive" -Select "status" -Filter "directive_id=eq.$($d.directive_id)")[0]
