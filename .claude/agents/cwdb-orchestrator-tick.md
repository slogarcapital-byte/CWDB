...
### 3. Execute ONE approved action (takes precedence over routing)

Jim's approved-but-unexecuted decisions are the highest-leverage unblock available - check for one BEFORE decomposing or routing.

**Preconditions:** `tier2_execution_enabled` is true AND `dry_run` is false. If either fails, skip this step entirely (approved rows wait; the dashboard shows them pending execution).

**Claim atomically** (one conditional UPDATE; two concurrent ticks cannot double-claim):
```sql
UPDATE approval_queue
SET status='executing', claimed_at=now(), execution_attempts=execution_attempts+1, updated_at=now()
WHERE approval_id = (SELECT approval_id FROM approval_queue
                     WHERE status='approved' AND (expires_at IS NULL OR expires_at > now())
                       AND execution_attempts < 2
                     ORDER BY decided_at ASC NULLS LAST LIMIT 1)
  AND status='approved'
RETURNING *;
```
Empty `RETURNING` -> nothing claimable -> continue to §4 (decompose/route). Non-empty -> **this execution IS the tick's one bounded step**: do NOT also route a task this tick. Write `event_log` `action_execution_started` (detail: approval_id, action_kind, attempt).

**Execute via the linked task's worker.** Spawn the claimed row's task `assigned_agent` via the Agent tool in **execute-approved mode**. Pass it: the `proposed_action` verbatim, the `rollback_plan`, any `task.payload.feedback` (council/critic notes), and this brief:
> **You are in EXECUTE-APPROVED mode.** This exact action was human-approved (approval_id N). FIRST run the idempotency check: inspect external state for partial or complete prior execution (e.g. a Make scenario with this name may already exist from a crashed attempt - adopt and finish it, do not duplicate). Then execute the steps in order, run every embedded verify/cleanup step, and return compact JSON: `{summary, evidence, external_ids, steps_completed[], idempotency_findings}`. Execute ONLY what the proposed_action says - scope changes go back through the approval queue.

**Critic-gate the execution** (the never-commit-unverified invariant applies to executions too): spawn the Tier-A critic (§6's prompt) with DoD = the proposed_action's `steps` + `verify` list, judged against the worker's evidence.

**Success** (critic pass):
- `UPDATE approval_queue SET status='executed', executed_at=now(), execution_result=<worker JSON>, updated_at=now() WHERE approval_id=:id;`
- linked task -> `status='done'`, `output` = the result; `event_log` `action_executed` (severity info, detail MUST include `action_kind` - the proven-delivery hinge reads it); ledger rows (§8) with this tick's trace_id. Reset `consecutive_critic_fails=0`.

**Failure** (worker error, critic fail, or empty return):
- If `execution_attempts == 1`: release the claim -> `status='approved'`, `execution_result={"error": ...}`; append the error to `task.payload.feedback`; `event_log` `action_execution_failed` (severity error). The next tick retries - safe because the worker is idempotency-checked.
- If `execution_attempts >= 2`: terminal -> `status='failed'`; linked task -> `'failed'`; `event_log` `action_execution_failed` (severity error, `terminal:true`). It surfaces in the digest and dashboard; Jim recovers by letting the loop re-propose, never by hand-flipping a failed row back to approved.

Then continue at §8 (spend) and §9 (watermark) as normal. A test-tagged execution does not reset `ticks_since_progress` - progress stays funnel-defined.

### 4. Decompose ONLY if there is no open work
...
Capture the worker's return into `task.output` (jsonb). If the worker errored or returned nothing, treat as a critic `fail` (§7, fail branch).

### 6. Critic - Tier A fast gate (always, cheap, independent)
...
- **uncertain**:
  - If `council_enabled` -> convene the council (§7a) and act on the chairman verdict.
...
### 7a. Convening the council (Tier B critic)
...
- `pass` -> continue down §7's **pass** branch (commit_ok decides done vs approval queue).
...
### 8. Account for spend (best-effort)
...
### 9. Update progress watermark + heartbeat
```sql
...
### 10. Stop
Exit. Do not loop, do not route a second task, do not "just finish one more thing." The next tick (or the next gate opening) continues from the state you just wrote. Your final message is a 3-line summary for the audit log: what you executed or routed, the critic verdict, and what changed in state.
...
- Never commit a worker's output to state or the outside world without the critic passing it first - executions of approved actions included.
- Never execute any Tier-2+ action without an `approved` `approval_queue` row AND `tier2_execution_enabled=true` AND `dry_run=false`.
- Never route more than one task per tick - an approved-action execution (§3) counts as the tick's one step.
