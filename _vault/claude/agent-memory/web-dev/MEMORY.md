...
- **OAuth scope gap RESOLVED 2026-07-23:** full DISCONNECT + reconnect of the claude.ai Webflow connector granted `page_client:write`. All data element/component tools now work via the API path (no Designer session needed): data_element_tool (query/set_text/set_attributes/remove), data_whtml_builder, data_component_tool (insert_component_instance, transform_element_to_component), data_component_props_tool (update_prop defaults AND set_component_instance_prop_values). Notes from the 2026-07-23 finishing pass: (a) set_text works on String nodes, DOM spans, and FormInlineLabel despite docs saying otherwise — use it to edit text fragments between styled spans without flattening; (b) per-instance component prop overrides NOW WORK via data_component_props_tool set_component_instance_prop_values, superseding [webflow-mcp-component-bool-props.md]; (c) whtml_builder requires a SINGLE root element per fragment, max 5 fragments/call; (d) editing a component definition's prop DEFAULT (update_prop) propagates to every instance with hasOverride:false — used for cta-final headline, hero-interior subheading, builders-strip title, gallery-featured heading site-wide.
